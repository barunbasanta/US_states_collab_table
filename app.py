...
def load_state_geometries():
    import glob
    shp_matches = (
        glob.glob("/opt/pyvenv/lib/python*/site-packages/mpl_toolkits/basemap_data/UScounties.shp")
        + glob.glob("/home/adminuser/venv/lib/python*/site-packages/mpl_toolkits/basemap_data/UScounties.shp")
    )
    if not shp_matches:
        return {}

    reader = shapefile.Reader(shp_matches[0].replace(".shp", ""), encoding="latin1")
    parts_by_state = defaultdict(list)

    for sr in reader.iterShapeRecords():
        abbr = sr.record.as_dict().get("STATE")
        if abbr in {"PR", "DC"}:
            continue
        pts = sr.shape.points
        ...
        for i in range(len(parts) - 1):
            ...
            try:
                ...
                if not poly.is_empty:
                    parts_by_state[abbr].append(poly)
            except Exception:
                pass

    return {k: unary_union(v).simplify(0.015, preserve_topology=True) for k, v in parts_by_state.items() if v}
...
def draw_icon(ax, geom, x0, y0, w, h, fill, linewidth=1.0):
    ...
    for g in geoms:
        if g.area < max_area * 0.003:
            continue
        arr = np.asarray(g.exterior.coords)
        xs = (arr[:, 0] - minx) * sx + x0 + w*0.06
        ys = (arr[:, 1] - miny) * sy + y0 + h*0.06
        ax.add_patch(MplPolygon(np.c_[xs, ys], closed=True, facecolor=fill,
                                edgecolor="#404040", linewidth=linewidth, joinstyle="round"))

...
def load_state_geometries():
    """
    Load bundled US state outline coordinates.

    This avoids bokeh.sampledata, basemap, shapely, and pyshp at runtime on
    Streamlit Cloud. State icons are still real state outlines, not boxes.
    """
    shapes_path = Path(__file__).parent / "us_state_shapes.json"
...
def draw_icon(ax, geom, x0, y0, w, h, fill, linewidth=1.0):
    """
    Draw a real state outline from bundled polygon parts.
    """
    if geom is None:
    ...
    all_xy = np.vstack(parts)
    minx, miny = np.nanmin(all_xy[:, 0]), np.nanmin(all_xy[:, 1])
    maxx, maxy = np.nanmax(all_xy[:, 0]), np.nanmax(all_xy[:, 1])
